...
func (i *Index) Create() types.Index {
	return &Index{}
}
...
func (i *Index) Traverse(orderColumns map[string]string, whereCallback types.WhereCallback, callback types.TraverseCallback) bool {
...
func (i *Index) GetColumnsForIndex(cond types.CompareConditions, order types.Order) ([]string, []string, map[string]bool) {
	allColumns := []string{}
...
func (i *Index) BuildIndex(storage storage.Storage, cond types.CompareConditions, order types.Order) types.Index {
	index := i.Create()
	...
	for key := range rows {
		index.Set(positions[key], rows[key])
	}
